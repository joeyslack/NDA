...
print(
    "load page with options. Category: ", category, " Page: ", page, " Count: ", count
)
...
while 1:
    driver.get(
        url=f"https://www.ewg.org/skindeep/browse/category/{category}/?page={page}&per_page={count}&sort=reverse-score"
    )

    time.sleep(1)
    start = driver.find_element(
        By.XPATH, "//section[contains(@class, 'product-listings')]"
    )
    ...
    for item in items:
        ...
        data2 = a.find_element(
            By.XPATH, ".//div[contains(@class, 'product-data-availability')]"
        )
        find = re.search(
            r"^https:\/\/www.ewg.org\/skindeep/products\/([0-9]+)",
        ...
        try:
            ...
            find = re.search(
                r"^https:\/\/www.ewg.org\/[a-zA-Z\-\_]+/[a-zA-Z\-\_]+\/([0-9]+)",
                a.get_attribute("href"),
            )
            if find is None or find.group is None or find.group(1) is None:
                print(
                    ...
                    a.get_attribute("href"),
                    data1.find_element(
                        By.XPATH, ".//div[contains(@class, 'product-company')]"
                    ).text,
                ),
                continue
            id = find.group(1)
        ...
        try:
            hazard = data2.find_element(
                By.XPATH,
                ".//div[contains(@class, 'product-score')]/div[contains(@class, 'verified-text')]",
            ).text
        except:
            hazard = data2.find_element(
                By.XPATH,
                ".//div[contains(@class, 'product-score')]/div/div[contains(@class, 'hazard-level')]",
            ).text
        ...
        insertValues.append(
            [
                ...
                data1.find_element(
                    By.XPATH, ".//div[contains(@class, 'product-name')]"
                ).text,
    ...
    curr.executemany(
        "INSERT INTO products(id, category, company, product, image, details, score) VALUES(%s, %s, %s, %s, %s, %s, %s) ON CONFLICT (id) DO UPDATE SET (category, company, product, image, details, score) = (%s, %s, %s, %s, %s, %s)",
        insertValues,
    )
    conn.commit()
